...
const OrgAddExitDetails = () => {
  ...
  const [selectInterViewId, setSelectInterViewId] = useState("");
  const { id } = useParams();
  ...
  const handleSelectInterViewId = (event) => {
    setSelectInterViewId(event.target.value);
  };
  ...
  console.log(
    selectInterViewId,
    selectUserId,
    "selectInterViewId,selectUserId"
  );
